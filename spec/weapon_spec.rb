  ...
  describe "#calculate_price" do
    context 'when it is a melee weapon' do
      ...
      it 'calculates the price without ammo quality, max range or reliability' do
        ...
        # 70,000 * 2 = 140,000
        expect(weapon.calculate_price).to eq(140000)
      end
    end

    context 'when it is a gun' do
      let(:weapon) { Gun.new(ammo: Ammo.new(damage_dice: 3 * D8, crit_multiplier: 4, quality: -1), max_range: 100, reliability: 1) }
      ...
      it 'calculates the price with ammo quality, max range and reliability' do
        expect(weapon.calculate_price).to eq(949200)
      end
    end
  end
end
